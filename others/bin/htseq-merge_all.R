...
print(paste0("############################"))
##Read files names
files <- list.files(path=path, pattern="*.count")
...
labs <- paste("", gsub("\\.count", "", files, perl=TRUE), sep="")
...
cov <- list()
for (i in labs) {
		filepath <- file.path(path,paste(i,".count",sep=""))
...
df <-Reduce(function(x,y) merge(x = x, y = y, by ="ENSEMBL_GeneID", all.x = TRUE, all.y = TRUE), cov)
df[is.na(df)] <- 0
...
write.table(df,paste(path, myoutname,".count",sep=""), sep="\t", quote= F, row.names = F)
